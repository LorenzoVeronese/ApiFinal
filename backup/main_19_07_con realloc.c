...
Ptr_queue* delete_element_priority_queue(Ptr_queue* ptr_queue, int to_remove, int* n);
...
Ptr_queue* delete_element_priority_queue(Ptr_queue* ptr_queue, int to_remove, int* n_queue){
	//Ptr_queue temp;
	//temp = ptr_queue[to_remove];
	ptr_queue[to_remove] -> key = ptr_queue[*n_queue - 1] -> key;
	ptr_queue[to_remove] -> dist = ptr_queue[*n_queue - 1] -> dist;
	//free(ptr_queue[*n_queue - 1]);
	//ptr_queue[*n_queue - 1] = NULL;
	*n_queue = *n_queue - 1;
	//free(ptr_queue[*n_queue]);
	ptr_queue = (Ptr_queue*)realloc(ptr_queue, *n_queue * sizeof(Ptr_queue));
...
int dijkstra(Ptr_node* ptr_arr, Ptr_queue* ptr_queue, int dest, int n){
	...
	while(1){
		...
		// il minimo.
		if(curr == dest){
			if(ptr_queue[0] -> dist == (int)INFINITY){
				return 0;
			}
			//D printf("%d\n", ptr_queue[0] -> dist);
			...
			while(curr_node != NULL){
				...
				if(index_reached != (int)INFINITY){
					...
					if(ptr_queue[index_reached] -> dist > cost){
						...
						// Dprint_priority_queue(ptr_queue, n_queue);
					}
				}
				curr_node = curr_node -> next;
			}
		...
		*/
		if(n_queue != 1){
			...
			ptr_queue = delete_element_priority_queue(ptr_queue, 0, &n_queue);
			// Dprintf("Tolgo il primo\n");
			// Dprint_priority_queue(ptr_queue, n_queue);
...
void clear_priority_queue(Ptr_queue* ptr_queue, int n){
	...
	ptr_queue = (Ptr_queue*)realloc(ptr_queue, n * sizeof(Ptr_node));
	ptr_queue[0] -> key = 0;
	ptr_queue[0] -> dist = 0;
